...
public class Aerolinea
{
    ...
    {
    	IPersistenciaAerolinea pesistenciaAerolinea = CentralPersistencia.getPersistenciaAerolinea(tipoArchivo);
    	pesistenciaAerolinea.cargarAerolinea(tipoArchivo, this);
    }
    ...
    {
    	IPersistenciaAerolinea pesistenciaAerolinea = CentralPersistencia.getPersistenciaAerolinea(tipoArchivo);
    	pesistenciaAerolinea.salvarAerolinea(tipoArchivo, this);
    }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	for(Avion avionActual: this.aviones) {
    		
    		if (avionActual.getNombre()== nombreAvion) {
    	...
    	for (Vuelo vueloActual: this.getVuelos()) {
    		
    		if((vueloActual.getAvion().getNombre() == nombreAvion)&&(vueloActual.getFecha()== fecha)) {
    			
    			Ruta rutaActual = vueloActual.getRuta();
    			int horaSalidaActual = Integer.parseInt(rutaActual.getHoraSalida());
    	    	int horaLlegadaActual = Integer.parseInt(rutaActual.getHoraLlegada());
    			
    			if((horaSalida < horaLlegadaActual) && (horaLlegada > horaSalidaActual)){
    	...
    	if (seCruza == false) {
    		...
    		throw new Exception("Avion no disponible");
    	}
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if (this.existeCliente(identificadorCliente)) {
    		...
    		throw new Exception("Cliente no exite");
    	}
    	
    	String[] mesesTemporadaBaja= {"enero", "febrero", "mayo", "septiembre", "octubre", "noviembre"};
    	
    	
    	CalculadoraTarifas calculadora = null;
    	boolean temporadaBaja = false;
			
		for (String mes : mesesTemporadaBaja) {
    ...
    {
    	this.vuelos.removeIf(v -> v.equals(this.getVuelo(codigoRuta, fecha)));
    }
